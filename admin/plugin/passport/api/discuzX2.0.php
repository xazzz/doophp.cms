...
/*
 * $dbconfig["X20"] = array("localhost", "phpwind", "root", "", "mysql", true, "collate"=>"utf8_unicode_ci", "charset"=>"utf8"); 
 ...
 * 
 * require_once LUA_ROOT.ADMIN_ROOT.'/plugin/passport/api/discuzX2.0.php';
 * $uc = new ucenter();
 * $this->user = $uc->user();
 */
...
class ucenter extends DooController{
    ...
    public $cookiepre = 'ZehO_';
    
    // 在 config/config_global.php 里查找 authkey
    public $authkey = '474d40Hh0HDWfRqH';
    
    // 在 config/config_global.php 里查找 tablepre
    public $tablepre = 'pre_';
    
    // 在 config/config_global.php 里查找 cookiepath
    public $cookiepath = '/';
    
    // 在 config/config_global.php 里查找 cookiedomain
    public $cookiedomain = '';
        ...
        if (isset($_DCOOKIE['auth'])){
            ...
            if($discuz_uid) {
                ...
                if(!empty($user) && $user['password'] == $discuz_pw) {
                    unset($user['password']);
                    $user['hash'] = substr(md5(substr(TIMESTAMP, 0, -7).iconv('utf-8','gbk',$user['username']).$discuz_uid.$authkey), 8, 8);
                    return $user;
                }
            }
        }
