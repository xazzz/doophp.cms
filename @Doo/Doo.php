...
class Doo{
    ...
    protected static function load($class_name, $path, $createObj=FALSE){
        if(is_string($class_name)===True){
            ...
            $_File = $path . "$class_name.php";
            if (!file_exists($_File)){
                $_File = LUA_ROOT.ADMIN_ROOT."/controller/$class_name.php";
            }
            class_exists($pure_class_name, false)===True || require_once($_File);
            if($createObj)
     ...
     * @return mixed returns NULL by default. If $createObj is TRUE, it creates and return the Object(s) of the class name passed in.
     */
    public static function loadClass($class_name, $createObj=FALSE, $path = ''){
    	if ($path){
    		return self::load($class_name, LUA_ROOT . $path . "/class/", $createObj);
    	}else{
